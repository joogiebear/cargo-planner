...
const CP_STORE = (() => {
  ...
  return {
    ...
    appendAudit: (entry) => {
      ...
      const next = [{ ...entry, t }, ...log].slice(0, 200);
      state['audit'] = next; save();
      listeners.forEach(fn => fn('audit', next));

      // Also persist to Supabase. Look up actor UUID via the legacy id we
      // already have in localStorage cache.
      const activeUserLegacy = state['activeUserId'];
      const users = window.CP_USERS || [];
      const actor = users.find(u => u.id === activeUserLegacy);
      appendAuditRow(entry, actor?._uuid).catch(err => {
        console.error('[supabase] audit append failed', err);
      });
    },
  };
})();
...
function App({ authedUser = null, onSignOut = null }) {
  ...
  // Active job: persist by ID, derive the full object from the live ships cache.
  // This way the user lands back where they were after a refresh, and the
  // object stays in sync when itemIds change underneath it.
  const [activeShipmentId, setActiveShipmentId] = usePersist('activeShipmentId', null);
  const [shipsCache, setShipsCache] = useState(() => CP_STORE.get('shipments', []) || []);
  useEffect(() => {
    return CP_STORE.subscribe((k) => {
      if (k === 'shipments' || k === null) {
        setShipsCache(CP_STORE.get('shipments', []) || []);
      }
    });
  }, []);
  const activeShipment = activeShipmentId
    ? shipsCache.find(s => s.id === activeShipmentId) || null
    : null;
  const setActiveShipment = useCallback((s) => {
    setActiveShipmentId(s?.id || null);
  }, [setActiveShipmentId]);
  ...
  useEffect(() => {
    window.CP_NAV = {
      ...
      // Add a freshly-created crate to the Load Plan items list. Called by the
      // "+ Add crates → New crate" flow on the shipment detail dialog.
      appendItem: (crate) => {
        setItems(prev => {
          if (prev.some(i => i.id === crate.id)) return prev;
          return [...prev, { ...crate, truckId: null, status: 'queued' }];
        });
      },
      // Drop a crate from the Load Plan items list (e.g., after deleting it
      // from a job's manifest). Truck assignments, if any, are also cleared.
      removeItem: (id) => {
  ...
  const [pickerUserId, setActiveUserId] = usePersist('activeUserId', 'u-02');
  // Real auth, when present, overrides the picker.
  const activeUser = authedUser || allUsers.find(u => u.id === pickerUserId) || allUsers[0];
  const activeUserId = activeUser?.id;
  // Keep CP_STORE.activeUserId in sync with the authenticated user so audit
  // appends and any other call sites reading it find the right actor.
  useEffect(() => {
    if (authedUser?.id) CP_STORE.set('activeUserId', authedUser.id);
  }, [authedUser?.id]);
  ...
  // Items list — every crate starts unassigned ('queued'). Drag onto a truck or
  // run Optimize to populate. Previously the prototype preloaded 12 demo crates
  // here, which confused real users into thinking the viz had stale data.
  ...
  // Load Plan is scoped to the active job. Items belonging to other jobs (or
  // crates with no job binding at all) are hidden from the planner.
  const scopedItems = useMemo(() => (
    activeShipment
      ? items.filter(i => activeShipment.itemIds.includes(i.id))
      : []
  ...
  const runFit = () => {
    const allItems = scopedItems.filter(i => i.status !== 'excluded');
  ...
  return (
    <div className="app" data-theme={resolvedTheme}>
        ...
        <div className="topbar-right">
          ...
          {authedUser ? (
            <span className="user-chip">
              ...
              <span className="muted small mono">· {authedUser.role}</span>
              {onSignOut && (
      ...
      <main className={`main ${t.density === 'dense' ? 'dense' : t.density === 'sparse' ? 'sparse' : ''} ${!canEditPlans ? 'role-readonly' : ''}`} data-role={activeUser?.role}>
        ...
        >
          ...
          <div className="pane-body">
            {!activeShipment ? (
              <div className="muted small" style={{ padding: 16, lineHeight: 1.6 }}>
                <strong>No job open.</strong><br />
                Open a job from the <button className="link-btn" onClick={() => setPage('shipments')}>Jobs</button> tab to see its crates here, or create a new job and add crates to it.
              </div>
            ) : filteredItems.length === 0 ? (
              <div className="muted small" style={{ padding: 16, lineHeight: 1.6 }}>
                <strong>This job has no crates yet.</strong><br />
                Click <em>+</em> above to add one, or <em>📋</em> to paste a list.
              </div>
            ) : (
            ...
            <div className="optimize-row">
              ...
              <div className="right">
                ...
                <button className="btn sm" onClick={async () => {
                  const scens = CP_STORE.get('scenarios', []);
                  const newScen = {
                  ...
                  const res = await saveScenario(newScen, activeUser?._uuid);
                  if (res.ok) {
                    CP_STORE.set('scenarios', [res.scenario, ...scens]);
                    CP_STORE.appendAudit({ who: activeUser?.name || 'Unknown', kind: 'edit', action: 'Saved scenario', target: res.scenario.name });
                    setToast('Scenario saved');
                  } else if (res.reason === 'no-config') {
                    // Local-only mode — still cache it so the UI updates
                    CP_STORE.set('scenarios', [newScen, ...scens]);
                    CP_STORE.appendAudit({ who: activeUser?.name || 'Unknown', kind: 'edit', action: 'Saved scenario (local only)', target: newScen.name });
                    setToast('Scenario saved (local only)');
                  } else {
      ...
      {/* Dialogs */}
      {showAdd && <AddDialog onClose={() => setShowAdd(false)} onAdd={async (it) => {
        // Same persistence path as the shipment-detail New crate flow:
        // INSERT to Supabase → updates window.CP_DATA.items → uses the
        // returned legacy_id (so the Jobs detail dialog can resolve it).
        const homeFacUuid = data.facilities?.find(f => f.id === facilityId)?._uuid;
        const res = await saveCrate({ ...it, _homeFacilityUuid: homeFacUuid });

        let crate;
        if (res.ok) {
          crate = res.crate;
        } else if (res.reason === 'no-config') {
          // Local-only mode — synthesize the crate locally so the UI moves on
          crate = { ...it, status: 'queued', truckId: null };
        ...
        setItems(prev => [...prev, { ...crate, truckId: null, status: 'queued' }]);
